...
if ( !function_exists( 'best_setup' ) ) {
    function best_setup() {
        ...
        function topbar_nav_fallback() {
            echo '<ul class="nav sf-menu"><li><a href="' . home_url() . '" title="Home">Home</a></li></ul>';
        }
...
add_action( 'wp_enqueue_scripts', 'load_best_styles' );

if ( !function_exists( 'load_best_styles' ) ) {
    function load_best_styles() {
        if ( !is_admin() ) {
            wp_register_style( 'best-styles', get_template_directory_uri() . '/style.css', array(), 1.0 );
            wp_enqueue_style( 'best-styles' );
        }
    }
    /* ===| end load_best_scripts() |=========================== */
}
...
add_action( 'wp_enqueue_scripts', 'load_best_scripts' );

if ( !function_exists( 'load_best_scripts' ) ) {
    function load_best_scripts() {
        if ( !is_admin() ) {
            wp_register_script( 'modernizr', get_template_directory_uri() . '/js/libs/modernizr-2.5.3.min.js' );
            wp_enqueue_script( 'modernizr' );
            wp_deregister_script( 'jquery' );
            wp_register_script( 'jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js', array(), 1.7, true );
            wp_enqueue_script( 'jquery' );
            wp_register_script( 'best-plugins', get_template_directory_uri() . '/js/plugins.js', array('jquery'), 1.0, true );
            wp_enqueue_script( 'best-plugins' );
            wp_register_script( 'best-scripts', get_template_directory_uri() . '/js/scripts.js', array('jquery'), 1.0, true );
            wp_enqueue_script( 'best-scripts' );
            wp_register_script( 'best-slider', get_template_directory_uri() . '/js/slider.js', array('jquery'), 1.0, true );
            if ( is_front_page() || is_home() ) wp_enqueue_script( 'best-slider' );
...
add_action( 'wp_head', 'best_theme_options' );

if ( !function_exists( 'best_theme_options' ) ) {
    function best_theme_options() {
    
        $background                = of_get_option( 'best_background', 'no entry' );
        $link_color_main           = of_get_option( 'best_link_color_main', 'no entry' );
        $link_color_hover_main     = of_get_option( 'best_link_hover_color_main', 'no entry' );
        $link_color_footer         = of_get_option( 'best_link_color_footer' );
        $link_color_hover_footer   = of_get_option( 'best_link_hover_color_footer' );
        $background_featured       = of_get_option( 'best_background_featured_content' );
        $text_color_featured       = of_get_option( 'best_text_color_featured_content' );
        $link_color_featured       = of_get_option( 'best_link_color_featured_content' );
        $link_color_hover_featured = of_get_option( 'best_link_hover_color_featured_content' ); ?>
